...
KNOWN_SYSTEMS = {*SYSTEMS, "low_lr_checkpoint_75"}
...
def selected_systems(args: argparse.Namespace | None = None) -> tuple[str, ...]:
    ...
    bad = [item for item in normalized if item not in KNOWN_SYSTEMS and not adapter_env_var(item)]
    if bad:
        raise SystemExit(f"BLOCKED: unsupported systems requested: {bad}; allowed={sorted(KNOWN_SYSTEMS)}")
    if len(normalized) < 2:
        raise SystemExit("BLOCKED: at least two systems are required for group comparison.")
    return normalized


def adapter_env_var(system: str) -> str:
    return os.environ.get(f"PASHU_ADAPTER_{system.upper()}", "").strip()
...
def generate_predictions(args: argparse.Namespace) -> dict[str, Any]:
    ...
    adapter_paths = resolve_adapter_paths(args, systems)
...
def resolve_adapter_paths(args: argparse.Namespace, systems: tuple[str, ...]) -> dict[str, Path | None]:
    paths: dict[str, Path | None] = {
        "base_gemma": None,
        "checkpoint_250": resolve_checkpoint_path(args.checkpoint_250, "checkpoint-250") if "checkpoint_250" in systems else None,
        "checkpoint_264": resolve_checkpoint_path(args.checkpoint_264, "checkpoint-264") if "checkpoint_264" in systems else None,
        "low_lr_checkpoint_75": resolve_named_adapter("low_lr_checkpoint_75", "checkpoint-75") if "low_lr_checkpoint_75" in systems else None,
    }
    for system in systems:
        if system in paths:
            continue
        env_path = adapter_env_var(system)
        paths[system] = resolve_named_adapter(system, system, explicit=env_path)
    return paths
...
def resolve_named_adapter(system: str, checkpoint_name: str, explicit: str = "") -> Path | None:
    if explicit:
        path = Path(explicit)
        if path.exists():
            return path
        raise SystemExit(f"BLOCKED: explicit adapter path for {system} does not exist: {path}")
    candidates = (
        Path("/kaggle/input/pashupulse-low-lr-checkpoint-75") / checkpoint_name,
        Path("/kaggle/input/pashupulse-low-lr-checkpoint-75") / system,
        Path("/kaggle/input/pashupulse-low-lr-from-ckpt250-v3/pashu_pulse_low_lr_from_ckpt250_v1") / checkpoint_name,
        Path("pashu_saathi/kaggle_outputs/low_lr_from_ckpt250_v3/pashu_pulse_low_lr_from_ckpt250_v1") / checkpoint_name,
    )
    for candidate in candidates:
        if (candidate / "adapter_config.json").exists() and (candidate / "adapter_model.safetensors").exists():
            return candidate
    search_roots = [Path("/kaggle/input"), Path("pashu_saathi/kaggle_packages")]
    for root in search_roots:
        if not root.exists():
            continue
        for adapter_config in sorted(root.rglob("adapter_config.json")):
            ...
            if (checkpoint_name.lower() in text or system.lower() in text) and (candidate / "adapter_model.safetensors").exists():
                return candidate
    return None
...
def choose_group_recommendation(winners: Counter[str], systems: tuple[str, ...] = SYSTEMS) -> str:
    if systems == ("checkpoint_250", "low_lr_checkpoint_75"):
        return "low_lr_checkpoint_75" if winners["low_lr_checkpoint_75"] > winners["checkpoint_250"] else "checkpoint_250"
    if systems == ("base_gemma", "low_lr_checkpoint_75"):
        return "low_lr_checkpoint_75" if winners["low_lr_checkpoint_75"] > winners["base_gemma"] else "base_gemma"
