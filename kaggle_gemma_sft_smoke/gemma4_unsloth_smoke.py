...
DATA_CANDIDATES = (
    "/kaggle/input/pashu-saathi-sft-smoke-package",
    "/kaggle/input/pashu-saathi-sft-package",
)
PINNED_PACKAGES = (
    "unsloth==2026.5.2",
...
MAX_LENGTH = 512
MAX_STEPS = 10
OUT_DIR = Path(os.environ.get("PASHU_SAATHI_OUT_DIR", "/kaggle/working/pashu_pulse_unsloth_smoke"))
...
def log_status(phase: str, step: int, total: int, **extra: Any) -> None:
    ...
    print(f"[pashu-smoke] phase={phase} step={step}/{total} left={row['remaining_steps']} {json.dumps(clean(extra), ensure_ascii=False, sort_keys=True, allow_nan=False)}", flush=True)
...
def validate_package(data_dir: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    required = ["sft_train.jsonl", "sft_dev.jsonl", "training_config.json", "sft_package_manifest.json"]
    missing = [name for name in required if not (data_dir / name).exists()]
    if missing:
        raise RuntimeError(f"missing smoke package files: {missing}")
    ...
    if manifest.get("package_mode") != "smoke":
        raise RuntimeError(f"smoke requires package_mode=smoke, got {manifest.get('package_mode')}")
    ...
    if bad:
        raise RuntimeError(f"eval-only/final rows leaked into smoke package: {bad[:5]}")
    return train[:12], dev[:4], manifest
...
def render_rows(rows: list[dict[str, Any]], tokenizer: Any) -> tuple[Any, dict[str, Any]]:
    ...
    for row in rows:
        ...
        if INSTRUCTION_PART not in text or RESPONSE_PART not in text:
            raise RuntimeError(f"Gemma turn markers missing in rendered row {row.get('row_id')}; prefix={text[:300]!r}")
        token_lengths.append(len(getattr(tokenizer, "tokenizer", tokenizer)(text, add_special_tokens=False)["input_ids"]))
        rendered.append({"text": text, "row_id": row.get("row_id", "")})
    audit = {
        "row_count": len(rendered),
        "row_ids": [row["row_id"] for row in rendered],
    ...
    return Dataset.from_list(rendered), audit
...
def main() -> None:
    ...
    train_rows, dev_rows, package_manifest = validate_package(data_dir)
    log_status("validated_package", 1, MAX_STEPS, data_dir=str(data_dir), model_path=str(model_path), train_rows=len(train_rows), dev_rows=len(dev_rows))
    ...
    disable_caching()
    gpu = gpu_report()
    log_status("validated_gpu", 2, MAX_STEPS, **gpu)
    ...
    smoke_summary = {"train": train_audit, "dev": dev_audit, "lora_target_count": len(lora_targets), "lora_targets": lora_targets[:40], **trainable_audit(model)}
    log_status("loaded_and_rendered", 3, MAX_STEPS, lora_target_count=len(lora_targets), **gpu_report())
    ...
    log_status("trainer_ready", 4, MAX_STEPS, **gpu_report())
    ...
    for item in trainer.state.log_history:
        if "loss" in item:
            ...
            log_status("training", step, MAX_STEPS, train_loss=item.get("loss"), learning_rate=item.get("learning_rate"), grad_norm=item.get("grad_norm"), **gpu_report())
    metrics = dict(result.metrics)
    train_loss = float(metrics.get("train_loss", 0.0))
    if not math.isfinite(train_loss):
        raise RuntimeError(f"non-finite train_loss={train_loss}")
    ...
    write_json(
        OUT_DIR / "metrics.json",
        {
            ...
            "adapter_tower_file_count": len([name for name in adapter_files if "vision_tower" in name or "audio_tower" in name]),
            **gpu_report(),
        },
    )
    write_json(
    ...
    log_status("completed", MAX_STEPS, MAX_STEPS, adapter_path=str(adapter_dir), elapsed_sec=round(time.time() - start, 2))
